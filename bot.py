...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username not in config.PV_USERS:
            bot.sendMessage(update.message.chat.id,'🚫Pídele acceso🛇 a mi desarrollador:@studio_apps_dev')
            return

        if config.getUser(username) is None:
            config.createUser(username)
            config.saveDB()
            reply_msg = '😃Bienvenido ' + username + ' usa este comando /tutorial 😃\n'
        ...
        if '/start' in msgText:
            start_msg = 'Bot :Moodle Upload 1.0\n'
            start_msg+= 'Desarrollador:@studio_apps_dev\n'
            start_msg+= 'para aprender a usar el bot usa el comando /tutorial\n'
            start_msg+= ' Estoy listo✅\n'
            ...
            bot.editMessageText(message,'error:no valido')
    except Exception as ex:
